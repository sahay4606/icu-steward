...
BAND_FILL = PatternFill("solid", start_color="F2F6F8", end_color="F2F6F8")

def write_sheet(wb, name, headers, rows, widths=None, note=None):
    ...
    if widths:
        ...
        for c in range(1, len(headers) + 1):
            maxlen = max([len(str(headers[c-1]))] + [len(str(r[c-1])) for r in rows]) if rows else len(str(headers[c-1]))
            ws.column_dimensions[get_column_letter(c)].width = min(max(maxlen * 0.9 + 3, 12), 48)
...
# 1. DIAGNOSIS CATALOG
# ----------------------------------------------------------------------------
diagnosis_headers = ["Diagnosis_ID","Diagnosis_Name","ICD10_Code","Category","Typical_Onset_Setting",
                      "Severity_Tier","Common_Pathogens","Empiric_Coverage_Class","Care_Bundle_Reference"]
diagnosis_rows = [
...
write_sheet(wb, "Diagnosis_Catalog", diagnosis_headers, diagnosis_rows,
            note="Master catalog of ICU-relevant diagnoses / clinical syndromes. Diagnosis is the hub entity: every other catalog links back to it through the Dx_*_Mapping tables.")

# ----------------------------------------------------------------------------
# 2. ANTIBIOTIC CATALOG
# ----------------------------------------------------------------------------
abx_headers = ["Antibiotic_ID","Drug_Name","Class","Spectrum_Category","Route","Renal_Adjustment",
               "Hepatic_Adjustment","Key_Monitoring","Allergy_Class","Primary_Indications","Spectrum_Notes"]
abx_rows = [
("AB01","Meropenem","Carbapenem","Broad gram-negative incl. Pseudomonas/ESBL","IV","Yes","No","Renal function, seizure threshold","Beta-lactam","VAP, severe sepsis, ESBL infections","Stable to most ESBL/AmpC enzymes"),
("AB02","Imipenem-Cilastatin","Carbapenem","Broad gram-negative/positive","IV","Yes","No","Renal function, seizure threshold (lower than meropenem)","Beta-lactam","Severe polymicrobial infection","Higher seizure risk than meropenem"),
("AB03","Ertapenem","Carbapenem","Broad, excludes Pseudomonas/Acinetobacter","IV/IM","Yes","No","Renal function","Beta-lactam","Intra-abdominal sepsis, complicated UTI","Once-daily dosing, not for Pseudomonas"),
("AB04","Piperacillin-Tazobactam","Beta-lactam/inhibitor combo","Broad gram +/- + anaerobes","IV","Yes","No","Renal function","Beta-lactam","Intra-abdominal sepsis, HAP, febrile neutropenia","Workhorse broad-spectrum agent"),
("AB05","Ampicillin-Sulbactam","Beta-lactam/inhibitor combo","Gram +/- + anaerobes (narrower)","IV","Yes","No","Renal function","Beta-lactam","Aspiration pneumonia, mild intra-abdominal infection","Narrower than pip-tazo"),
("AB06","Ceftriaxone","3rd-gen cephalosporin","Gram-negative + some gram-positive","IV/IM","No (hepatobiliary clearance)","Yes","LFT","Beta-lactam","CAP, meningitis, uncomplicated infections","No anti-pseudomonal activity"),
("AB07","Ceftazidime","3rd-gen cephalosporin (anti-pseudomonal)","Gram-negative incl. Pseudomonas","IV","Yes","No","Renal function","Beta-lactam","Pseudomonas-targeted therapy","Weak gram-positive activity"),
("AB08","Cefepime","4th-gen cephalosporin","Broad incl. Pseudomonas","IV","Yes","No","Renal function, neurotoxicity in renal failure","Beta-lactam","Febrile neutropenia, HAP/VAP","Neurotoxicity risk if underdosed in renal failure"),
("AB09","Ceftaroline","5th-gen cephalosporin","Gram-positive incl. MRSA + some gram-negative","IV","Yes","No","Renal function","Beta-lactam","Complicated SSTI/CAP with MRSA risk","Only cephalosporin with MRSA activity"),
("AB10","Ceftazidime-Avibactam","Cephalosporin/beta-lactamase inhibitor","MDR gram-negative incl. KPC-CRE","IV","Yes","No","Renal function","Beta-lactam","Carbapenem-resistant Enterobacterales, MDR Pseudomonas","No activity vs metallo-beta-lactamases"),
("AB11","Ceftolozane-Tazobactam","Cephalosporin/inhibitor combo","MDR Pseudomonas, gram-negative","IV","Yes","No","Renal function","Beta-lactam","MDR Pseudomonas infections","Potent anti-pseudomonal activity"),
("AB12","Meropenem-Vaborbactam","Carbapenem/inhibitor combo","CRE-targeted","IV","Yes","No","Renal function, seizure threshold","Beta-lactam","KPC-producing CRE","Targets KPC specifically"),
("AB13","Aztreonam","Monobactam","Gram-negative only","IV","Yes","No","Renal function","None (safe in most penicillin allergy)","Gram-negative infection in beta-lactam-allergic patients","Shares side chain with ceftazidime - caution if allergic to that"),
("AB14","Vancomycin","Glycopeptide","Gram-positive incl. MRSA","IV","Yes (levels)","No","Trough/AUC, creatinine","Rare true allergy","MRSA, CLABSI, severe gram-positive infection","Red-man syndrome is infusion-rate reaction, not allergy"),
("AB15","Linezolid","Oxazolidinone","Gram-positive incl. VRE, MRSA","IV/PO","No","No","CBC/platelets weekly, serotonin syndrome risk","Rare","VRE, MRSA pneumonia","Caution with serotonergic drugs"),
("AB16","Daptomycin","Lipopeptide","Gram-positive incl. MRSA, VRE","IV","Yes","No","CPK weekly","Rare","Bacteremia, endocarditis","Inactivated by pulmonary surfactant - not for pneumonia"),
("AB17","Colistin","Polymyxin","MDR gram-negative (CRE, Acinetobacter)","IV","Yes (nephrotoxic)","No","Renal function, electrolytes","Rare","Carbapenem-resistant organisms, MDR organisms","Last-line agent, narrow therapeutic window"),
("AB18","Polymyxin B","Polymyxin","MDR gram-negative","IV","Yes (nephrotoxic)","No","Renal function, neurotoxicity","Rare","Carbapenem-resistant organisms (alternative to colistin)","Dosing not renally adjusted the way colistin is"),
("AB19","Amikacin","Aminoglycoside","Gram-negative incl. Pseudomonas","IV","Yes (levels)","No","Peak/trough, renal function, hearing","Rare","MDR synergy/combination therapy","Often combined with beta-lactam, not monotherapy"),
("AB20","Gentamicin","Aminoglycoside","Gram-negative, enterococcal synergy","IV","Yes (levels)","No","Peak/trough, renal function, hearing","Rare","Synergy in endocarditis, gram-negative infection","Synergy dosing differs from treatment dosing"),
("AB21","Ciprofloxacin","Fluoroquinolone","Gram-negative","IV/PO","Yes","No","QTc, tendon","Rare","UTI, atypical pneumonia","Weak gram-positive activity"),
("AB22","Levofloxacin","Fluoroquinolone","Gram-negative + atypicals + some gram-positive","IV/PO","Yes","No","QTc, tendon","Rare","CAP, atypical coverage","Better gram-positive activity than ciprofloxacin"),
("AB23","Metronidazole","Nitroimidazole","Anaerobes","IV/PO","No","Yes","LFT, neuro symptoms with prolonged use","Rare","Intra-abdominal infection, C. difficile","No aerobic gram-negative or gram-positive activity"),
("AB24","Clindamycin","Lincosamide","Gram-positive, anaerobes, toxin suppression","IV/PO","No","Yes","C. difficile risk","Rare","Necrotizing fasciitis (toxin suppression), anaerobic infection","Used adjunctively for toxin-mediated disease"),
("AB25","Tigecycline","Glycylcycline","Broad incl. anaerobes, excl. Pseudomonas/Proteus","IV","No","Yes (severe hepatic impairment)","LFT","Tetracycline-related","Complicated intra-abdominal/skin infection","Low serum concentrations - avoid for bacteremia"),
("AB26","Trimethoprim-Sulfamethoxazole","Folate antagonist combo","Gram-negative, some gram-positive, Stenotrophomonas","IV/PO","Yes","No","Renal function, hyperkalemia, CBC","Sulfa","UTI, Stenotrophomonas, PJP","First-line for Stenotrophomonas maltophilia"),
("AB27","Doxycycline","Tetracycline","Atypicals, broad spectrum","IV/PO","No","Yes","Photosensitivity","Tetracycline","Atypical pneumonia, tick-borne illness","Avoid in pregnancy, young children"),
("AB28","Fluconazole","Azole antifungal","Candida spp. (non-resistant)","IV/PO","Yes","Yes","LFT, QTc","Rare","Candidemia (susceptible species)","No activity vs C. glabrata (variable), C. krusei (resistant)"),
("AB29","Voriconazole","Azole antifungal","Aspergillus, some Candida","IV/PO","No","Yes","LFT, drug levels, visual disturbance","Rare","Invasive aspergillosis","Numerous drug interactions, requires level monitoring"),
("AB30","Micafungin","Echinocandin antifungal","Broad Candida incl. resistant species","IV","No","No","LFT","Rare","First-line candidemia/invasive candidiasis","Preferred empiric choice for unstable candidemia"),
("AB31","Amphotericin B (liposomal)","Polyene antifungal","Broad fungal incl. molds","IV","Yes (nephrotoxic)","No","Renal function, K/Mg, infusion reactions","Rare","Refractory/severe invasive fungal infection","Reserved for resistant or refractory infections"),
]
write_sheet(wb, "Antibiotic_Catalog", abx_headers, abx_rows,
            note="Master catalog of antibiotics/antifungals used in ICU stewardship workflows, with organ-adjustment flags and monitoring requirements.")

# ----------------------------------------------------------------------------
# 3. INVESTIGATION CATALOG
# ----------------------------------------------------------------------------
inv_headers = ["Investigation_ID","Investigation_Name","Type","Specimen_Method","Clinical_Purpose","Typical_Frequency","Linked_Entities"]
inv_rows = [
...
write_sheet(wb, "Investigation_Catalog", inv_headers, inv_rows,
            note="Master catalog of labs and imaging used across diagnosis workup, drug monitoring, and device surveillance.")

# ----------------------------------------------------------------------------
# 4. DEVICE CATALOG
# ----------------------------------------------------------------------------
dev_headers = ["Device_ID","Device_Name","Insertion_Site_Type","Primary_Infection_Risk","Recommended_Cultures",
               "Recommended_Monitoring","Review_Trigger","Typical_Removal_Criteria","Review_Interval"]
dev_rows = [
...
write_sheet(wb, "Device_Catalog", dev_headers, dev_rows,
            note="Master catalog of invasive ICU devices with infection risk, surveillance cultures, and removal/review criteria.")

# ----------------------------------------------------------------------------
# 5. CULTURE CATALOG
# ----------------------------------------------------------------------------
cul_headers = ["Culture_ID","Culture_Name","Specimen_Source","Associated_Devices","Typical_Turnaround","Common_Organisms","Notes"]
cul_rows = [
...
write_sheet(wb, "Culture_Catalog", cul_headers, cul_rows,
            note="Master catalog of culture/specimen types, their associated devices, and typical organisms recovered.")

# ----------------------------------------------------------------------------
# 6. ORGANISM CATALOG
# ----------------------------------------------------------------------------
org_headers = ["Organism_ID","Organism_Name","Category","Typical_Source_Site","Notable_Resistance_Concern","Notes"]
org_rows = [
...
write_sheet(wb, "Organism_Catalog", org_headers, org_rows,
            note="Master catalog of clinically relevant ICU organisms with resistance concerns.")

# ----------------------------------------------------------------------------
# 7. Dx_Antibiotic_Mapping
# ----------------------------------------------------------------------------
h = ["Diagnosis_ID","Diagnosis_Name","First_Line_Empiric_Antibiotic(s)","Escalation_Option","Typical_Duration_Range","Notes"]
rows = [
...
write_sheet(wb, "Dx_Antibiotic_Mapping", h, rows,
            note="Empiric antibiotic selection per diagnosis. IDs in parentheses reference Antibiotic_Catalog. These are general stewardship-reference ranges, not a substitute for institutional protocols or ID consultation.")

# ----------------------------------------------------------------------------
# 8. Dx_Investigation_Mapping
# ----------------------------------------------------------------------------
h = ["Diagnosis_ID","Diagnosis_Name","Recommended_Investigations","Notes"]
rows = [
...
write_sheet(wb, "Dx_Investigation_Mapping", h, rows, note="Recommended diagnostic workup bundle per diagnosis.")

# ----------------------------------------------------------------------------
# 9. Dx_Device_Mapping
# ----------------------------------------------------------------------------
h = ["Diagnosis_ID","Diagnosis_Name","Associated_Devices","Relevance"]
rows = [
...
write_sheet(wb, "Dx_Device_Mapping", h, rows, note="Devices causally or diagnostically linked to a given diagnosis.")

# ----------------------------------------------------------------------------
# 10. Dx_Culture_Mapping
# ----------------------------------------------------------------------------
h = ["Diagnosis_ID","Diagnosis_Name","Required_Cultures","Notes"]
rows = [
...
write_sheet(wb, "Dx_Culture_Mapping", h, rows, note="Required culture orders per diagnosis.")

# ----------------------------------------------------------------------------
# 11. Abx_Culture_Mapping
# ----------------------------------------------------------------------------
h = ["Antibiotic_ID","Antibiotic_Name","Triggering_Culture","Organism_Suggestive_Of","Recommended_Action"]
rows = [
...
write_sheet(wb, "Abx_Culture_Mapping", h, rows, note="Culture/organism results that confirm, narrow, or change an antibiotic choice.")

# ----------------------------------------------------------------------------
# 12. Abx_Investigation_Mapping
# ----------------------------------------------------------------------------
h = ["Antibiotic_ID","Antibiotic_Name","Required_Investigation","Purpose","Frequency"]
rows = [
...
write_sheet(wb, "Abx_Investigation_Mapping", h, rows, note="Monitoring investigations required while a given antibiotic is active.")

# ----------------------------------------------------------------------------
# 13. Abx_Device_Mapping
# ----------------------------------------------------------------------------
h = ["Device_ID","Device_Name","Suspected_Source","Empiric_Antibiotic","Rationale"]
rows = [
...
write_sheet(wb, "Abx_Device_Mapping", h, rows, note="Empiric antibiotic choice driven by a suspected device source, before culture results return.")

# ----------------------------------------------------------------------------
# 14. Organism_Susceptibility
# ----------------------------------------------------------------------------
h = ["Organism_ID","Organism_Name","Typically_Susceptible_To","Typically_Resistant_Avoid","Notes"]
rows = [
...
write_sheet(wb, "Organism_Susceptibility", h, rows,
            note="Typical susceptibility patterns by organism. Always confirm against the institutional antibiogram and individual susceptibility report - these are general patterns, not patient-specific guidance.")

# ----------------------------------------------------------------------------
# 15. Allergy_CrossReactivity
# ----------------------------------------------------------------------------
h = ["Allergy_ID","Allergy_Type","Cross_Reactive_Classes","Risk_Level","Safe_Alternatives","Notes"]
rows = [
...
write_sheet(wb, "Allergy_CrossReactivity", h, rows,
            note="Cross-reactivity guidance for antibiotic allergy/intolerance documentation. Severity of index reaction should always be confirmed before assuming class-wide avoidance.")

# ----------------------------------------------------------------------------
# 16. Device_Investigation_Mapping
# ----------------------------------------------------------------------------
h = ["Device_ID","Device_Name","Required_Investigation","Purpose","Frequency"]
rows = [
...
write_sheet(wb, "Device_Investigation_Mapping", h, rows, note="Investigations triggered by the presence of a specific device.")

# ----------------------------------------------------------------------------
# 17. Device_Culture_Mapping
# ----------------------------------------------------------------------------
h = ["Device_ID","Device_Name","Linked_Culture","Trigger"]
rows = [
...
write_sheet(wb, "Device_Culture_Mapping", h, rows, note="Culture orders automatically suggested when a device-related infection is suspected.")

# ----------------------------------------------------------------------------
# 18. Investigation_Culture_Mapping
# ----------------------------------------------------------------------------
h = ["Investigation_ID","Investigation_Name","Linked_Culture","Rationale"]
rows = [
...
write_sheet(wb, "Investigation_Culture_Mapping", h, rows, note="Investigation findings that trigger or accompany a culture order.")

# ----------------------------------------------------------------------------
# 19. Culture_Organism_Mapping
# ----------------------------------------------------------------------------
h = ["Culture_ID","Culture_Name","Likely_Organism","Likelihood_Tier","Pathogen_or_Contaminant_Note"]
rows = [
...
write_sheet(wb, "Culture_Organism_Mapping", h, rows,
            note="Expected organism likelihood by culture/specimen type, flagged for pathogen-vs-contaminant interpretation.")

# ----------------------------------------------------------------------------
# 20. Stewardship_Review_Triggers
# ----------------------------------------------------------------------------
h = ["Trigger_ID","Trigger_Condition","Recommended_System_Action","Linked_Entity_Type","Escalation_Path"]
rows = [
...
write_sheet(wb, "Stewardship_Review_Triggers", h, rows,
            note="Operational business rules that turn the static catalogs/mappings above into the daily stewardship review loop described in the refined flow.")

# ----------------------------------------------------------------------------
# README (created last, moved to front)
# ----------------------------------------------------------------------------
readme_headers = ["Sheet_Name","Purpose","Primary_Key","Links_To"]
readme_rows = [
("Diagnosis_Catalog","Master list of ICU diagnoses/clinical syndromes - severity, pathogens, care bundle reference","Diagnosis_ID","Dx_Antibiotic/Investigation/Device/Culture_Mapping"),
("Antibiotic_Catalog","Master list of antibiotics/antifungals - class, spectrum, organ adjustments, monitoring","Antibiotic_ID","Abx_Culture/Investigation/Device_Mapping, Organism_Susceptibility, Allergy_CrossReactivity"),
("Investigation_Catalog","Master list of labs/imaging used across workup, monitoring, and surveillance","Investigation_ID","Dx_Investigation_Mapping, Abx_Investigation_Mapping, Device_Investigation_Mapping"),
("Device_Catalog","Master list of invasive ICU devices - infection risk, review/removal criteria","Device_ID","Dx_Device_Mapping, Abx_Device_Mapping, Device_Investigation/Culture_Mapping"),
("Culture_Catalog","Master list of culture/specimen types and expected organisms","Culture_ID","Dx_Culture_Mapping, Abx_Culture_Mapping, Device_Culture_Mapping, Culture_Organism_Mapping"),
("Organism_Catalog","Master list of clinically relevant ICU organisms with resistance notes","Organism_ID","Organism_Susceptibility, Culture_Organism_Mapping"),
("Dx_Antibiotic_Mapping","Empiric antibiotic choice per diagnosis","Diagnosis_ID + Antibiotic_ID","Diagnosis_Catalog, Antibiotic_Catalog"),
("Dx_Investigation_Mapping","Recommended workup per diagnosis","Diagnosis_ID + Investigation_ID","Diagnosis_Catalog, Investigation_Catalog"),
("Dx_Device_Mapping","Devices causally/diagnostically linked to a diagnosis","Diagnosis_ID + Device_ID","Diagnosis_Catalog, Device_Catalog"),
("Dx_Culture_Mapping","Required cultures per diagnosis","Diagnosis_ID + Culture_ID","Diagnosis_Catalog, Culture_Catalog"),
("Abx_Culture_Mapping","Culture/organism results that confirm, narrow, or change an antibiotic","Antibiotic_ID + Culture_ID","Antibiotic_Catalog, Culture_Catalog"),
("Abx_Investigation_Mapping","Monitoring investigation required while an antibiotic is active","Antibiotic_ID + Investigation_ID","Antibiotic_Catalog, Investigation_Catalog"),
("Abx_Device_Mapping","Empiric antibiotic choice driven by a suspected device source","Device_ID + Antibiotic_ID","Device_Catalog, Antibiotic_Catalog"),
("Organism_Susceptibility","Typical susceptibility pattern of each organism to antibiotic classes","Organism_ID","Organism_Catalog, Antibiotic_Catalog"),
("Allergy_CrossReactivity","Cross-reactivity rules for antibiotic allergy/intolerance","Allergy_ID","Antibiotic_Catalog"),
("Device_Investigation_Mapping","Investigations triggered by device presence","Device_ID + Investigation_ID","Device_Catalog, Investigation_Catalog"),
("Device_Culture_Mapping","Cultures triggered by device-related infection suspicion","Device_ID + Culture_ID","Device_Catalog, Culture_Catalog"),
("Investigation_Culture_Mapping","Investigation findings that trigger a culture order","Investigation_ID + Culture_ID","Investigation_Catalog, Culture_Catalog"),
("Culture_Organism_Mapping","Likelihood of organisms recovered by culture/specimen type, with contaminant flags","Culture_ID + Organism_ID","Culture_Catalog, Organism_Catalog"),
("Stewardship_Review_Triggers","Daily/event-based business rules that operationalize the stewardship review loop","Trigger_ID","All entities"),
]
write_sheet(wb, "README", readme_headers, readme_rows,
            note=None)
# Move README to front and add a title block
ws = wb["README"]
wb.move_sheet("README", offset=-(len(wb.sheetnames)-1))
ws.insert_rows(1, amount=3)
ws.cell(row=1, column=1, value="ICU Antimicrobial Stewardship - Data Dictionary").font = TITLE_FONT
ws.cell(row=2, column=1, value="Single source of truth for diagnosis, antibiotic, investigation, device, culture, and organism entities, plus every relational mapping between them.").font = SUB_FONT
...
out_path = os.path.join(out_dir, "ICU_Antimicrobial_Stewardship_Data_Dictionary.xlsx")
wb.save(out_path)
