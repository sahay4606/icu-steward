...
import { X, FlaskConical, Search } from 'lucide-react-native';
import { radius, spacing, typography } from '../theme';
import { useThemeColors } from '../contexts/ThemeContext';
import { API_BASE_URL } from '../lib/config';
import { normalizeKeys } from '../lib/format';

export function InvestigationPickerModal({ visible, onClose, onSelect, selectedName }) {
  ...
  useEffect(() => {
    ...
    const match = items.find((d) => d.investigationName === selectedName);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`${API_BASE_URL}/api/investigation-master`)
      .then((r) => { if (!r.ok) throw new Error('Failed to load investigations'); return r.json(); })
  ...
  const categories = useMemo(() => {
    const set = new Set();
    for (const d of items) { if (d.category) set.add(d.category); }
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter((d) => d.investigationName?.toLowerCase().includes(q) || d.category?.toLowerCase().includes(q));
    }
  ...
  const showCustom = search.trim().length > 0 && !items.some((d) => d.investigationName?.toLowerCase() === search.trim().toLowerCase());
      ...
      <View style={styles.overlay}>
        <View style={styles.modal}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>Select Investigation</Text>
              <Text style={styles.subtitle}>{items.length} investigations in master list</Text>
            </View>
            <TouchableOpacity onPress={onClose} style={styles.closeBtn} data-testid="inv-picker-close">
          ...
          <View style={styles.searchRow}>
            ...
            <TextInput
              ...
              placeholder="Search or type custom investigation..."
              placeholderTextColor={colors.text.tertiary}
              data-testid="inv-picker-search"
            />
          </View>
            ...
            <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
              {showCustom ? (
                <TouchableOpacity
                  activeOpacity={0.7}
                  onPress={() => {
                    onSelect?.({ investigationName: search.trim(), category: '', isCustom: true });
                    onClose();
                  }}
                  style={[styles.item, styles.itemCustom]}
                  data-testid="inv-picker-custom"
                >
                  <FlaskConical color={colors.status.warning} size={16} strokeWidth={2} />
                  <View style={{ flex: 1 }}>
                  ...
                  {catItems.map((d) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setSelectedId(d.id);
                          onSelect?.({ investigationName: d.investigationName, category: d.category, isCustom: false });
                          onClose();
                        }}
                        style={[styles.item, isPicked && styles.itemSelected]}
                        data-testid={`inv-picker-${d.investigationName?.toLowerCase().replace(/\s+/g, '-')}`}
                      >
                        <View style={[styles.radio, isPicked && styles.radioSelected]}>
                          {isPicked ? <View style={styles.radioDot} /> : null}
                        </View>
                        <View style={{ flex: 1 }}>
                          <Text style={[styles.itemName, isPicked && styles.itemNameSelected]}>{d.investigationName}</Text>
                          {d.sampleType ? <Text style={styles.itemMeta}>Sample: {d.sampleType}</Text> : null}
                        </View>
                      </TouchableOpacity>
                    );
                  })}
              ...
              {filtered.length === 0 && !showCustom && (
                <Text style={styles.empty}>No investigations match "{search}"</Text>
              )}
            </ScrollView>
          )}
        </View>
